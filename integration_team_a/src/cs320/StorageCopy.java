 ...
 *
 */
public class StorageCopy extends Storage {
...
//##		}
	...
	boolean Push() {
		...
		input1.put("p_id","1");
		input1.put("first_name","Tyler");
		input1.put("last_name","Bonci");
		input1.put("dob","1989-01-25");
		input1.put("mrn","54521");
		input1.put("cmrn","1245");
		input1.put("sex","male");
		input1.put("has_note","124821");
		input1.put("is_editable","false");
		
		input2.put("p_id","2");
		input2.put("first_name","Tylere");
		input2.put("last_name","Boncie");
		input2.put("dob","1989-01-23");
		input2.put("mrn","545");
		input2.put("cmrn","125");
		input2.put("sex","female");
		input2.put("has_note","21");
		input2.put("is_editable","true");
		ReturnList.add(input1);
		ReturnList.add(input2);
		//##ReturnList = Retrieve("cs320.patient", null); /*<-- **NEED TO MAKE STUB** */
		...
		for (int i = 0; i < ReturnList.size(); i++) {// No longer doubles, maybe
			// I was going crazy...
			...
			//##testSaves[i] = ServerUpdate(input);/*<-- **NEED TO MAKE STUB** */
			testSaves[i]=true;
			input.clear();
		}
		...
		if (!ClientConnected || !ServerConnected) {
			...
			try {
				...
				if (ClientVersion.equalsIgnoreCase(ServerVersion)) {
					...
					createReturn = ClientStatement.executeUpdate(createQuery);
				//##	if (result != 0) {
				//##		return false;
					//##}
